...
baseline, the seven HT-series contacting decks -- HT-01A Hinge, HT-01B
Spring, HT-01C LipSeal, HT-02 CVS, HT-03 GRADEX, HT-05 PULSAR and HT-08
VORTEXA -- and the HyTrays Apex flagship) against every service case in
...
"paper"-style write-up comparing the tray types service by service.
...
def write_report(all_results, matrix, path: str) -> None:
    ...
    a("The conventional **Sieve** baseline, the seven HyTrays HT-series "
      "contacting decks -- **HT-01A Hinge, HT-01B Spring, HT-01C LipSeal, "
      "HT-02 CVS, HT-03 GRADEX, HT-05 PULSAR and HT-08 VORTEXA** -- and the "
      "**HyTrays Apex** flagship are sized and rated with the Fair-correlation "
      "tray-hydraulics engine in `column_hydraulics.py` across five "
      "representative distillation services. The goal is to make the "
    ...
    apex_hpl = next((r for r in hpl if r.tray.name == "HyTrays Apex"), None)
    apex_ranks = {SHORT_NAME[s]: matrix["HyTrays Apex"][s][0]
                   for s, *_ in RANKING_SPECS} if "HyTrays Apex" in matrix else None
    if apex_hpl is not None and apex_ranks is not None:
        a(f"- **HyTrays Apex** (Integrated Cartridge-Grid flagship, *above* "
          f"the HT-series line -- see `HyTrays_Apex_concept.md`) tops the "
          f"selection matrix in General [{apex_ranks['General']}], Fouling "
          f"[{apex_ranks['Fouling']}], Vacuum [{apex_ranks['Vacuum']}] and "
          f"Foaming [{apex_ranks['Foaming']}], trading a tighter downcomer "
          f"margin in High-P/High-L [{apex_ranks['High-P/High-L']}] for the "
          f"smallest shell there too ({apex_hpl.diameter_ft:.2f} ft vs "
          f"{sieve_hpl.diameter_ft:.2f} ft for Sieve, at "
          f"{apex_hpl.dc_backup_pct:.0f}% of the 50% backup limit -- still "
          f"inside the limit but the tightest in the family). As a "
          f"*synthesis* design combining five HT-0X mechanisms, these numbers "
          f"are a first-pass design target pending CFD/pilot validation, more "
          f"provisional than the individual HT-0X figures above.")
    ...
    a("- **HyTrays Apex** is a *synthesis* design (concept memo Section 9): "
      "each of its parameters is built by combining the corresponding HT-0X "
      "figure, not from its own datasheet, so its numbers are a first-pass "
      "design target rather than a measured result -- treat them as more "
      "provisional than the individual HT-0X figures above.")
    a("")
    a("---")
