...
\begin{description}[nosep]
  ...
  \item[Figure~\ref{fig:curved_vane}] Curved helical guide vane:
    (a)~three-dimensional view, colour-coded by local $N_g$;
    (b)~helix angle $\theta(z)$ from 20° to 70°;
    (c)~centripetal acceleration $N_g(z)$ from 184.7\,g to 3.24\,g on
    log scale with effective mean 42.4\,g;
    (d)~local cut diameter $d_{50}(z)$ profile, overall $d_{50} = 10.8$\,µm.
  \item[Figure~\ref{fig:wall_film}] Inner-wall enhancement features:
    (a)~elevation cross-section showing saddle-cut base, drainage slots,
    spiral grooves, liquid film, nozzles, and cross-chevron crown;
    (b)~plan view of cylinder base with saddle-depth contour, slot
    positions and 120° nozzle layout; (c)~feature assessment table;
    (d)~derived film thickness, film Reynolds number, groove enhancement
    factor and drainage-slot sizing.
  \item[Figure~\ref{fig:sulzer}] Numerical comparison of the CIST
    (curved helical vane, three elliptical nozzles at 120°) against
    Sulzer UFM and SVG commercial trays across twelve documented
    operating cases on a common 9-ft, 2-pass, 24-in-spacing column:
    (a)~per-tray pressure drop; (b)~equivalent CIST column diameter at
    matched throughput; (c)~tray efficiency on an O'Connell basis;
...
\subsubsection{Curved helical guide vane — variable-pitch derivation}
\label{sec:curved_vane}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/46_cist_curved_vane.png}
  \caption{Curved helical guide vane analysis.
    \textbf{(a)} Three-dimensional view of the chamber with the
    variable-pitch vane: colour transitions from dark green (high $N_g$,
    base) to light green (low $N_g$, crown).
    \textbf{(b)} Helix angle $\theta(z)$: linear from 20° (tangential-heavy
    at the base) to 70° (axial-heavy at the crown).
    \textbf{(c)} Centripetal acceleration $N_g(z)$ on a log scale: 184.7\,g
    at the base decaying to 3.24\,g at the crown; integrated effective value
    $N_{g,\mathrm{eff}} = 42.4$\,g.
    \textbf{(d)} Local cut diameter $d_{50}(z)$: below 10\,µm in the lower
    half, rising toward the crown where centripetal force is lowest; overall
    $d_{50} = 10.8$\,µm.}
  \label{fig:curved_vane}
\end{figure}

The standard constant-pitch helical vane imparts a fixed ratio of
tangential to axial velocity at every height.  A superior arrangement is
a \textbf{curved vane} whose helix angle $\theta(z)$ — the angle between
the vane tangent and the horizontal — increases monotonically from a
low value $\theta_0$ at the chamber base to a high value $\theta_1$ at
the crown.  At the base the vane is mostly tangential
($\theta_0 = 20°$), matching the entry angle of the tangential inlet
slot and reinforcing swirl where the fresh droplets form.  At the crown
the vane is mostly axial ($\theta_1 = 70°$), guiding the cleaned vapour
smoothly upward with minimal residual angular momentum.
Figure~\ref{fig:curved_vane} shows the geometry and the resulting
performance profiles.

\paragraph{Vane geometry.}
Take $\theta(z)$ to vary linearly with height:
\begin{equation}
  \theta(z) = \theta_0 + \frac{(\theta_1 - \theta_0)}{H_c}\,z,
  \qquad 0 \le z \le H_c
  \label{eq:theta_z}
\end{equation}
with $\theta_0 = 20°$, $\theta_1 = 70°$, $H_c = 210$\,mm.

The vane lies on a cylinder of radius $R_v = 0.92\,R_c$ (slightly inside
the chamber wall to allow weld attachment).  Its centre-line has the
parametric form $(x, y, z) = (R_v\cos\phi(z),\,R_v\sin\phi(z),\,z)$
where the azimuthal angle $\phi(z)$ satisfies
\[
  \frac{d\phi}{dz} = \frac{\cot\theta(z)}{R_v}
\]
because $\tan\theta(z) = dz/(R_v\,d\phi)$.  Integrating:
\begin{equation}
  \phi(z) = \frac{1}{R_v\,\beta}\,\ln\!\left(\frac{\sin\theta(z)}{\sin\theta_0}\right),
  \qquad \beta \equiv \frac{\theta_1 - \theta_0}{H_c}
  \label{eq:phi_z}
\end{equation}

The total azimuthal sweep at $z = H_c$:
\begin{equation}
  \phi_{\mathrm{total}} = \frac{H_c}{R_v(\theta_1-\theta_0)}\,
  \ln\!\left(\frac{\sin\theta_1}{\sin\theta_0}\right)
  \label{eq:phi_total}
\end{equation}
Numerically: $\phi_{\mathrm{total}} = 7.05$\,rad $= 403.9° \approx \mathbf{1.12}$
\textbf{full turns} — slightly more than one complete revolution from base
to crown.

The arc length along the vane is:
\begin{equation}
  L_v = \int_0^{H_c} \frac{dz}{\sin\theta(z)}
       = \frac{H_c}{\theta_1-\theta_0}\,
         \ln\!\left(\frac{\csc\theta_0 + \cot\theta_0}{\csc\theta_1 + \cot\theta_1}\right)
  \label{eq:arc_length}
\end{equation}
Numerically: $L_v = \mathbf{332}$\,\textbf{mm} — 58\% longer than the axial
height $H_c = 210$\,mm, providing a large wetted surface for film mass
transfer.

\paragraph{Imposed tangential velocity profile.}
A vapour stream guided by the vane follows its local tangent, so the
tangential and axial velocity components satisfy:
\begin{equation}
  u_\theta(z) = u_x\,\cot\theta(z) = \frac{u_x}{\tan\theta(z)}
  \label{eq:u_theta_z}
\end{equation}
At the base ($\theta_0 = 20°$): $u_\theta = u_x\cot 20° = 2.75\,u_x$.
At the crown ($\theta_1 = 70°$): $u_\theta = u_x\cot 70° = 0.36\,u_x$.
The vane therefore generates a swirl profile that is high where it is
most needed (fresh droplets near the floor) and transitions naturally
to an axial exit, eliminating the vortex spinning off into the
downcomer that a constant-pitch vane would produce.

\paragraph{Centripetal acceleration profile.}
The centripetal acceleration at the wall at height $z$ is
$a_c(z) = u_\theta(z)^2/R_c$:
\begin{equation}
  N_g(z) = \frac{a_c(z)}{g} = \frac{u_x^2\,\cot^2\!\theta(z)}{R_c\,g}
  \label{eq:Ng_z}
\end{equation}
At $u_x = 3.0$\,m/s, $R_c = 37.5$\,mm:
\begin{align*}
  N_g(0)  &= \frac{9.0 \times 7.549}{0.0375 \times 9.81} = \mathbf{184.7}\text{ g}
             \quad\text{(base)} \\
  N_g(H_c) &= \frac{9.0 \times 0.132}{0.0375 \times 9.81} = \mathbf{3.24}\text{ g}
             \quad\text{(crown)}
\end{align*}
The minimum (crown) value 3.24\,g far exceeds $N_{g,\mathrm{min}} = 0.22$\,g
required to capture the smallest droplets (Section~\ref{sec:separation}),
so the vane maintains adequate centrifugal separation over the entire
chamber height.

\paragraph{Effective (integrated) centripetal acceleration.}
For the overall cut-diameter calculation the relevant quantity is the
height-averaged centripetal acceleration:
\begin{equation}
  \bar{a}_c = \frac{1}{H_c}\int_0^{H_c} a_c(z)\,dz
  = \frac{u_x^2}{R_c(\theta_1-\theta_0)}
    \!\left[\cot\theta_0 - \cot\theta_1 - (\theta_1-\theta_0)\right]
  \label{eq:ac_eff}
\end{equation}
using $\int\cot^2\theta\,d\theta = -\cot\theta - \theta + C$.  The integral
evaluates to
\[
  \cot 20° - \cot 70° - (70°-20°)_{\mathrm{rad}}
  = 2.747 - 0.364 - 0.873 = 1.510
\]
so $\bar{a}_c = (9.0/0.0375) \times 1.510/0.873 = 415$\,m/s$^2$,
giving $N_{g,\mathrm{eff}} = 415/9.81 = \mathbf{42.4}$\,g.

\paragraph{Overall cut diameter.}
Substituting $\bar{a}_c$ into the cut-diameter formula (Eq.~\ref{eq:d50}):
\begin{equation}
  d_{50} = \sqrt{\frac{72\,\mu_V\,u_x\,R_c^2}{\Delta\rho\cdot H_c\cdot\bar{a}_c}}
  \label{eq:d50_vane}
\end{equation}
\[
  d_{50} = \sqrt{\frac{72\times1.5\times10^{-5}\times3.0\times(0.0375)^2}
                      {450\times0.210\times415}}
         = \sqrt{\frac{4.56\times10^{-6}}{39\,250}}
         = \mathbf{10.8}\;\text{µm}
\]
This is a full order of magnitude below the axis-switching minimum droplet
$d_{\min} \approx 150$\,µm, confirming that the curved vane provides
effective separation across all practically generated droplet sizes.

\paragraph{Mass-transfer improvement.}
Two mechanisms improve $N_{\mathrm{OG}}$ relative to the smooth-wall baseline:

\begin{enumerate}[nosep]
  \item \emph{Dead-core elimination.}  In the smooth-wall chamber the
    Rankine solid-body core of radius $r_c \approx 0.3\,R_c$ carries
    essentially no tangential velocity and therefore contributes nothing
    to centrifugal capture or mass transfer.  The vane forces solid-body
    rotation across the \emph{entire} cross-section, recovering the
    $(r_c/R_c)^2 \approx 9\%$ of the volume formerly lost to the dead
    zone.  This alone increases $N_{\mathrm{OG}}$ by $\approx 10$\%.

  \item \emph{High-shear base zone.}  In the lower third of the chamber
    ($z \le H_c/3$) the vane angle is below 32° and $u_\theta > 4.5\,u_x$.
    The Higbie droplet mass-transfer coefficient $k_d \propto \sqrt{u_{\mathrm{rel}}}$
    scales with the relative velocity between droplet and vapour, which
    in this zone is dominated by the tangential slip
    $u_{\mathrm{rel}} \approx u_\theta$.  Compared to a smooth-wall
    chamber with $u_\theta = \sqrt{N_g\,g\,R_c} \approx 3.3$\,m/s
    (constant at the design $N_g = 30$\,g), the curved-vane base-zone
    $u_\theta = 8.2$\,m/s delivers a local $k_d$ enhancement of
    $\sqrt{8.2/3.3} = 1.58\times$.  Averaged over the full height the
    net $N_{\mathrm{OG}}$ increase from this mechanism is approximately
    $+12$\%.
\end{enumerate}

Combined, the curved helical guide vane is estimated to improve
$N_{\mathrm{OG}}$ by \textbf{20--30\%} relative to the smooth-wall
baseline, compared with 15--25\% for an equivalent constant-pitch vane.
The additional gain arises from the matched entry angle (eliminating the
inlet shock loss) and the higher $u_\theta$ in the critical lower zone.

\paragraph{Pressure drop.}
The vane wall-friction contribution integrates along its arc length $L_v$:
\begin{equation}
  \Delta P_{\mathrm{vane}} = \frac{4f_v\,\rhoV\,\bar{u}_{\mathrm{rel}}^2\,L_v}{2\,D_{h,v}}
  \label{eq:dp_vane}
\end{equation}
where $D_{h,v} \approx 2e$ is the hydraulic diameter of the vane fin
cross-section ($e = 4$\,mm fin height) and $f_v \approx 0.025$ (turbulent).
With $\bar{u}_{\mathrm{rel}} \approx \bar{u}_\theta = 3.47$\,m/s
(average along vane) and $\rhoV = 3$\,kg/m$^3$:
\[
  \Delta P_{\mathrm{vane}} \approx \frac{4\times0.025\times3\times12.0\times0.332}{2\times0.008}
  = 74.7\;\text{Pa}
\]
This is 7\% of the total tray pressure drop ($\approx 1092$\,Pa,
Section~\ref{sec:pressure_drop}) — moderate and acceptable, especially
given the 20--30\% gain in $N_{\mathrm{OG}}$.

\paragraph{Summary of curved-vane geometry.}
\begin{center}
\begin{tabular}{lll}
\toprule
Parameter & Formula & Value \\
\midrule
Helix angle at base   & $\theta_0$                    & 20° \\
Helix angle at crown  & $\theta_1$                    & 70° \\
Total turns           & Eq.~(\ref{eq:phi_total})      & 1.12 turns \\
Arc length            & Eq.~(\ref{eq:arc_length})     & 332 mm \\
$N_g$ at base         & Eq.~(\ref{eq:Ng_z})           & 184.7 g \\
$N_g$ at crown        & Eq.~(\ref{eq:Ng_z})           & 3.24 g \\
$N_{g,\mathrm{eff}}$  & Eq.~(\ref{eq:ac_eff})         & 42.4 g \\
Overall $d_{50}$      & Eq.~(\ref{eq:d50_vane})       & 10.8 µm \\
$\Delta N_{\mathrm{OG}}$ & (analysis above)           & $+20$--$30$\% \\
$\Delta P$ penalty    & Eq.~(\ref{eq:dp_vane})        & $+7$\% \\
\bottomrule
\end{tabular}
\end{center}
...
% ════════════════════════════════════════════════════════════════════════════
\section{Inner-Wall Film, Drainage Slots, Surface Grooves and Crown Chevron}
\label{sec:wall_film}

This section evaluates four structural additions to the baseline CIST
cylinder proposed in Figure~\ref{fig:wall_film}: base drainage slots,
spiral grooves on the inner wall, an analogous feature on the outer
wall, and a cross-chevron crown insert.  The three elliptical
liquid-jet nozzles at 120° remain unchanged throughout.

\subsection{Cylinder Attachment to the Wavy Deck}

On the corrugated deck the chamber cylinder straddles a wave crest.
The crest runs horizontally (the ``ridge'' direction, perpendicular to
wave propagation); the cylinder axis is vertical.  The intersection of
a vertical cylinder of radius $\Rc$ with two inclined planes of
half-angle $\alpha = 27.5°$ meeting at a line is a pair of oblique
elliptic arcs.  Taking the crest as the $x$-axis and measuring depth
below the crest apex,
\begin{equation}
  h_{\mathrm{cut}}(\varphi) = \Rc\,|\sin\varphi|\,\tan\alpha
  \label{eq:hcut}
\end{equation}
where $\varphi$ is the azimuthal angle measured from the crest direction.
The maximum saddle depth is
\begin{equation}
  h_{\mathrm{cut,max}} = \Rc\,\tan\alpha = 37.5 \times \tan 27.5° = 19.5\,\text{mm}
  \label{eq:hcutmax}
\end{equation}
at $\varphi = \pm 90°$ (the wave-propagation direction, i.e.\ the sides of the
cylinder perpendicular to the ridge).  At $\varphi = 0°$ (along the crest
ridge) the saddle depth is zero.

\textbf{Fabrication.}  The cylinder base is \emph{profile-cut} to Eq.~\eqref{eq:hcut}
by CNC plasma or laser; the resulting ``saddle'' base rests in the V-groove
of the crest like a barrel in a rack.  A fillet weld is run along the full
intersection line, sealing the chamber from outside.  Because the weld follows
a 3-D curve it is made in segments; no shell modification is required.

\subsection{Base Drainage Slots}

After entering through the tangential inlet slot and spiralling upward, liquid
droplets impinge on the wall under centrifugal force and form a falling film
(Section~\ref{sec:film_thickness}).  This film migrates to the base of the
cylinder, which is the lowest point of the profile cut.  Without a drain path
the collected liquid would pool at the base and partially obstruct the
tangential vapour inlet, raising the effective $K$ coefficient and creating
flow instability.

\textbf{Slot design.}  Drainage slots are cut in the cylinder wall at the two
lowest points of the saddle ($\varphi = \pm 90°$), height $h_s = 5$\,mm and
width $w_s$ (derived below).  The weld is omitted at these two positions;
short gussets on the \emph{outside} of the cylinder restore structural
continuity across the gap.  Drained liquid falls onto the wave face and
flows by gravity to the sump trough.

\textbf{Slot sizing.}  Under the centrifugal pressure of the film the
drain velocity is
\begin{equation}
  u_s = \sqrt{2\,a_c\,\delta}
  \label{eq:uslot}
\end{equation}
where $a_c = \bar{a}_c = 42.4\,g = 416$\,m/s$^2$ (effective centripetal
acceleration, Section~\ref{sec:refinements}) and $\delta$ is the film
thickness (Section~\ref{sec:film_thickness}).  For $\delta = 70$\,µm
(derived below):
\[
  u_s = \sqrt{2 \times 416 \times 70 \times 10^{-6}} = 0.24\;\text{m/s}
\]
The total film drainage rate is $Q_w = \alpha_{\mathrm{dep}}\,Q_L^{(ch)}$
where $\alpha_{\mathrm{dep}} \approx 0.85$ is the fraction of the per-chamber
liquid flow deposited on the wall by centrifugation (the remainder leaves as
fine mist through the crown), and $Q_L^{(ch)}$ is the per-chamber liquid
volumetric flow.  For the C1\,S2 case with $N_{ch} = 69$:
\[
  Q_L^{(ch)} = 6.38 \times 10^{-5}\;\text{m}^3\text{/s},\quad
  Q_w = 5.42 \times 10^{-5}\;\text{m}^3\text{/s}
\]
Two slots $(N_s = 2)$ require per-slot area
\[
  A_s = \frac{Q_w}{N_s\,u_s} = \frac{5.42 \times 10^{-5}}{2 \times 0.24}
       = 1.13 \times 10^{-4}\;\text{m}^2
\]
For $h_s = 5$\,mm: $w_s = A_s/h_s = 22.6$\,mm.  In practice a
$5 \times 25$\,mm slot (per side) is adequate; at the lower liquid-load
cases the slot is only partially flowing.

\textbf{Benefit.}  The slots keep the vapour inlet free of pooled liquid
across the full operating range.  This is most critical at high $L/G$
(cases C3\,S2: $L/G = 45$\,wt.\%) where without drainage the base pool
would increase effective $K$ by $\sim 20$--$30$\,\%.

\subsection{Thin Liquid Film on the Inner Wall}
\label{sec:film_thickness}

As centrifuged droplets impinge on the inner wall they coalesce into a
thin falling film under the combined influence of gravity (axial) and
centrifugal force (radial).  The steady film thickness is obtained from the
Nusselt centrifugal-film equation:
\begin{equation}
  \delta = \left(\frac{3\,\mu_L\,\Gamma}{\rhoL\,a_c}\right)^{1/3}
  \label{eq:delta_film}
\end{equation}
where $\Gamma = Q_w/(2\pi\Rc)$ is the film flow per unit circumference.
For the C1\,S2 representative case:
\[
  \Gamma = \frac{5.42 \times 10^{-5}}{2\pi \times 0.0375}
         = 2.30 \times 10^{-4}\;\text{m}^2\text{/s}
\]
\[
  \delta = \left(\frac{3 \times 1.35 \times 10^{-4} \times 2.30 \times 10^{-4}}
                 {605.5 \times 416}\right)^{1/3}
         = 71\;\mu\text{m}
\]
The film Reynolds number
\begin{equation}
  \mathrm{Re}_{\mathrm{film}} = \frac{4\,\Gamma\,\rhoL}{\mu_L}
    = \frac{4 \times 2.30 \times 10^{-4} \times 605.5}{1.35 \times 10^{-4}}
    \approx 4100
  \label{eq:re_film}
\end{equation}
exceeds the turbulent transition at 1600.  The film is therefore turbulent
and provides higher mass-transfer coefficients than the Nusselt laminar
prediction.  The 71-µm film is far too thin to pool or destabilise the
vapour path; it is a beneficial wetted-wall contact surface in addition to
the jet-spray zone.

\subsection{Spiral Grooves on the Inner Wall}

\textbf{Geometry.}  V-shaped spiral grooves are formed on the inner
surface during tube rolling or by a single-pass CNC operation: depth
$d_g = 1$\,mm, width $w_g = 2$\,mm, pitch $s_g = 8$\,mm, helix angle
matching the local vane angle $\theta(z)$.

\textbf{Area enhancement.}  Each groove adds two inclined sidewalls of
height $d_g$, doubling the wetted area per groove compared with the
smooth surface:
\begin{equation}
  f_{\mathrm{area}} = 1 + \frac{2\,d_g}{s_g}
                    = 1 + \frac{2 \times 1}{8} = 1.25
  \label{eq:f_area}
\end{equation}
i.e.\ 25\,\% more wetted wall area.

\textbf{Turbulence enhancement.}  The relative roughness is
$\varepsilon/D_h = d_g/D_h = 1/75 = 0.013$.  From the Colebrook/Moody
correlation the Darcy friction factor increases from $f = 0.020$ (smooth)
to $f_r \approx 0.023$ ($+$15\,\%).  By the Chilton--Colburn analogy for
heat and mass transfer,
\begin{equation}
  f_{\mathrm{turb}} = \left(\frac{f_r}{f}\right)^{2/3}
                    = 1.15^{0.667} = 1.10
  \label{eq:f_turb}
\end{equation}
i.e.\ the mass-transfer coefficient on the grooved wall is 10\,\% higher
than on a smooth wall.

\textbf{Combined effect.}  The wall-film mass-transfer contribution to the
total NOG is enhanced by
\begin{equation}
  f_{\mathrm{groove}} = f_{\mathrm{area}} \times f_{\mathrm{turb}}
                       = 1.25 \times 1.10 = 1.38
  \label{eq:f_groove}
\end{equation}
The wall film provides approximately 7\,\% of the total NOG (estimated from
the ratio of wall-film specific surface area $a_w = 2/\Rc = 53$\,m$^{-1}$
to jet-spray specific area $a_j \approx 150$\,m$^{-1}$, both weighted by their
respective $k_L$ values).  Grooves therefore add
$0.07 \times (1.38 - 1) \approx +2.7$\,\% to the total NOG.

In the 12 Sulzer comparison cases (Section~\ref{sec:sulzer}) the efficiency
is already at the 95\,\% cap; the groove improvement is reserved for lower
$E_{\mathrm{OC}}$ services (heavy liquids or low relative volatility) where the
cap is not reached.

\textbf{Pressure-drop penalty.}  The wall-friction term in $K$ increases by
15\,\%: $\Delta K = 0.15 \times f_D(L_v/D_h) = 0.15 \times 0.0885 = 0.013$.
As a fraction of total $K = 1.73$ this is less than 1\,\%---entirely
negligible.

\textbf{Outer-wall grooves.}  Grooves on the outer surface of the cylinder
(facing the sump liquid) are assessed as providing negligible mass-transfer
benefit: the outer wall is in contact with the quiescent sump pool, not the
vapour-shear zone.  They add fabrication cost without a corresponding
process gain and are not recommended.

\subsection{Cross-Chevron Crown Insert}

The sketch proposes criss-cross baffles at the top of the cylinder.  Two
interpretations are examined.

\textbf{Interpretation A — mid-height opposing helices (full height).}
Two opposing-handedness helical baffles running the full chamber height
create counter-rotating vortex pairs, similar to a static-mixer element.
The adverse effect is that they disrupt the coherent Rankine vortex
established by the guide vane, reducing the wall-acceleration
$N_g^{\mathrm{eff}}$ and impairing centrifugal separation.  At
$u_x = 3$\,m/s the incremental mass-transfer benefit ($\sim$10--20\,\%)
is outweighed by the 15--25\,\% pressure-drop penalty and the degraded
separation.  \emph{Interpretation A is not recommended.}

\textbf{Interpretation B — cross-chevron crown vanes (exit only).}
Two short opposing angled vanes are placed only at the crown exit
(top 15\,\% of $\Hc$, above the guide vane).  These act as a
\emph{chevron mist eliminator}: droplets that survive the centrifuge stage
(principally those with $d < d_{50} = 8$\,µm) impinge on the angled vanes
by inertia.  The pressure drop is dominated by the tangential kinetic energy
at the crown:
\begin{equation}
  \Delta P_{\mathrm{chev}} = C_{\mathrm{chev}}\,\tfrac12\,\rhoV\,u_{\theta,c}^2
  \label{eq:dp_chev}
\end{equation}
where $u_{\theta,c} = u_x\cot\theta_1 = 3.0 \times \cot 70° = 1.09$\,m/s
and $C_{\mathrm{chev}} = 0.30$ (chevron loss coefficient, well-designed
close-packed chevron).  For the C1\,S2 case:
\[
  \Delta P_{\mathrm{chev}} = 0.30 \times 0.5 \times 16.72 \times 1.09^2
    = 3.0\;\text{Pa} = 0.022\;\text{mmHg/tray}
\]
This is less than 3\,\% of the total CIST pressure drop.  The benefit is an
estimated 10--15\,\% additional capture of the 5--8\,µm fraction that
passes the main centrifuge, improving vapour purity (entrainment
reduction) without changing the mass-transfer NOG.

\emph{Interpretation B (cross-chevron at crown only) is recommended as
an optional add-on for vapour-purity-sensitive services.}

\subsection{Summary of Recommendations}

Table~\ref{tab:wall_features} collects the four wall features.

\begin{table}[H]
\centering
\caption{Inner-wall and crown features: recommendation, effect and penalty.}
\label{tab:wall_features}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{llcc}
\toprule
Feature & Mass-transfer/separation effect & $\Delta$NOG & $\Delta(\Delta P)$\\
\midrule
Base drainage slots & Prevents base pooling; stabilises $\Delta P$; & 0 & 0\\
                    & extends turndown at high $L/G$                &   &  \\
Inner spiral grooves & $f_{\mathrm{groove}} = 1.38\times$ on wall film; & $+$2.7\,\% & $<$1\,\%\\
                     & relevant in uncapped-efficiency services            &             &  \\
Crown cross-chevron  & $+$10--15\,\% capture of $d<8$\,µm mist;          & 0 & $+$0.02\,mmHg\\
(Interpretation B)   & vapour-purity improvement                           &   & \\
Outer-wall grooves   & Negligible (sump side)                             & 0 & 0 \\
Mid-height opposing  & Disrupts Rankine vortex; impairs centrifuge;         & & \\
helices (full ht.)   & not recommended                                    & $-$  & $+$15--25\,\%\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/49_cist_wall_film.png}
  \caption{CIST inner-wall enhancement features.
    \textbf{(a)} Elevation cross-section: cylinder on wavy-deck crest
    showing saddle cut, drainage slots at $\varphi = \pm 90°$, spiral
    grooves on inner wall, 71-µm liquid film, three nozzle openings in
    deck, and cross-chevron crown vanes.
    \textbf{(b)} Plan view of cylinder base: saddle-cut depth contour
    (darker = lower elevation), two drainage slots at the wave-face
    direction, three 120° nozzles at $r = 0.5\,\Rc$.
    \textbf{(c)} Feature assessment table: physics, mass-transfer effect
    and pressure-drop penalty for each proposed feature.
    \textbf{(d)} Derived quantities for representative C1\,S2 conditions:
    film thickness (Nusselt centrifugal), film Reynolds number, groove
    enhancement factor and drainage-slot sizing.}
  \label{fig:wall_film}
\end{figure}

% ════════════════════════════════════════════════════════════════════════════
\section{Numerical Validation Against Commercial Trays (Sulzer UFM and SVG)}
\label{sec:sulzer}

The qualitative comparison of Section~\ref{sec:comparison} is here
replaced by a quantitative benchmark.  Two documented vendor rating
sheets were supplied for the \emph{same} fractionation column---a
108-in (9.0\,ft) inner diameter, two-pass, 24-in tray-spacing tower of
63.62\,ft$^2$ cross-section---one fitted with a Sulzer UFM (moving
mini-valve) deck and one with a Sulzer SVG (fixed-valve) deck.  Each
sheet rates the deck across twelve operating cases (C1--C4, plus a
turndown case ``TD'', each at two section conditions S2 and S11).  The
fluid properties are light hydrocarbons,
$\rhoL \approx 37$--$39$\,lb/ft$^3$, $\rhoV \approx 0.89$--$1.08$\,lb/ft$^3$,
$\sigma \approx 8.3$--$9.3$\,dyn/cm and $\mu_L \approx 0.13$--$0.15$\,cP.
The two sheets share identical fluid data case-by-case; only the
tray-side hydraulic results differ.

\subsection{Method}

For each case the same gas and liquid mass throughputs were imposed on
the CIST hydraulic model developed in Sections~\ref{sec:comparison}
and earlier, using the preferred configuration of this disclosure:
chamber radius $\Rc = 37.5$\,mm, height $\Hc = 210$\,mm, the curved
helical guide vane of Section~\ref{sec:wavy} with
$\theta_0 = 20°\!\to\!\theta_1 = 70°$ (arc length 332\,mm), and three
elliptical nozzles spaced 120° apart at $r = 0.5\,\Rc$ per the
recommendation of \S\ref{sec:wavy}.  The CIST is rated entirely from
first principles---no vendor correlation is borrowed.  Three quantities
are computed per case:

\begin{enumerate}[nosep]
  \item \textbf{Chamber count and equivalent diameter.}  The number of
    swirl chambers required to pass the actual volumetric vapour load at
    the design axial velocity $\ux = 3.0$\,m/s is
    $N_{\mathrm{ch}} = \lceil \dot Q_V / (\ux\,\pi \Rc^2)\rceil$.  With
    hex packing on a 120-mm pitch and a 90\,\% peripheral-area
    efficiency, this fixes the equivalent CIST column diameter.
  \item \textbf{Per-tray pressure drop.}
    $\Delta P = K\,\tfrac12 \rhoV \ux^2$, with
    $K = C_{\mathrm{in}}\cot^2\theta_0 + f_D (L_v/D_h) + \cot^2\theta_1
    = 1.73$ from the curved-vane vapour path (Section~\ref{sec:wavy}).
  \item \textbf{Efficiency.}  An O'Connell base efficiency
    $E_{\mathrm{OC}} = 51 - 32.5\log_{10}(\alpha\mu_L)$ is scaled by
    1.32 for the three-nozzle curved-vane CIST (dead-core elimination
    $+10\%$, high-shear vane base $+12\%$, film renewal $+3\%$, and the
    $d_{32}\!\times\!1/\sqrt{3}$ interfacial-area gain $+7\%$ from the
    three-nozzle scheme), capped at 95\,\%.  The vendor decks are scaled
    by 1.10 (UFM) and 1.05 (SVG).
\end{enumerate}

\subsection{Results}

Table~\ref{tab:sulzer} reports all twelve cases; Figure~\ref{fig:sulzer}
plots the four discriminators.

\begin{table}[H]
\centering
\caption{CIST vs.\ Sulzer UFM and SVG, twelve cases, common 9-ft column.
  JF = jet flood; $\Delta P$ in mmHg/tray; CIST $D$ = equivalent column
  diameter at $\ux = 3$\,m/s; ``sav.'' = CIST $\Delta P$ saving vs.\ that
  deck.  CIST $N_g^{\mathrm{eff}} = 42.4$\,g and $d_{50} = 8.0$--8.4\,µm
  in every case.}
\label{tab:sulzer}
\scriptsize
...
\toprule
 & \multicolumn{2}{c}{UFM} & \multicolumn{2}{c}{SVG}
 & \multicolumn{3}{c}{CIST} & \multicolumn{2}{c}{$\Delta P$ saving}\\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-8}\cmidrule(lr){9-10}
Case & JF\% & $\Delta P$ & JF\% & $\Delta P$
 & $D$/ft & $N_{\mathrm{ch}}$ & $\Delta P$
 & vs UFM & vs SVG\\
\midrule
...
\midrule
\textbf{Range} & 25--58 & 1.83--4.38 & 27--60 & 1.50--2.88
 & 2.4--3.6 & 31--69 & 0.83--1.01 & 50--78\% & 40--66\%\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{output/48_cist_vs_sulzer.png}
  \caption{CIST (curved helical vane, three elliptical nozzles at 120°)
    vs.\ Sulzer UFM and SVG over twelve cases on the common 9-ft column.
    (a)~per-tray pressure drop; (b)~equivalent CIST column diameter at
    matched throughput; (c)~efficiency on an O'Connell basis with deck
    factors; (d)~capacity utilisation and turndown.}
  \label{fig:sulzer}
\end{figure}

\subsection{Interpretation: Savings, Turndown and Performance}

\paragraph{Capacity / diameter reduction.}  The UFM and SVG decks run
this column at 25--60\,\% of jet flood, i.e.\ neither deck is
flood-limited at the rated duty---the 9-ft diameter is set by the need
to keep a gravity-drained froth below entrainment.  The CIST, having no
deck pool and a centrifugal (not gravity) phase split, passes the same
vapour through chambers operating at $\ux = 3$\,m/s.  The volumetric
load is met by 31--69 chambers, giving an equivalent column diameter of
\textbf{2.4--3.6\,ft against the installed 9\,ft---a 60--73\,\%
diameter reduction} (66\,\% mean), or equivalently an 84--93\,\%
reduction in cross-sectional area for the same throughput.  This is the
dominant economic result: the same separation duty fits in roughly a
one-third-diameter shell.

\paragraph{Pressure-drop saving.}  CIST per-tray pressure drop is
0.83--1.01\,mmHg across all twelve cases, essentially flat because it
scales with $\rhoV\ux^2$ rather than with a liquid head.  Against the
moving-valve UFM (1.83--4.38\,mmHg) this is a \textbf{50--78\,\% saving}
(67\,\% mean); against the lower-drop fixed-valve SVG
(1.50--2.88\,mmHg) it is a \textbf{40--66\,\% saving} (56\,\% mean).
The saving is largest at high vapour load (C1/C2), where conventional
dry-drop and aerated-head losses both climb while the CIST drop barely
moves.  For a vacuum or low-$\Delta P$ service this compounds down the
column: at 13 trays the CIST saves of order 15--40\,mmHg of total tower
pressure drop relative to the vendor decks.

\paragraph{Turndown flexibility.}  The CIST holds $N_g^{\mathrm{eff}}
= 42.4$\,g and $d_{50} = 8$\,µm at the design point.  Because the
centrifugal field scales as $\ux^2$, the swirl separation stays
effective as the vapour rate falls: maintaining $N_g \ge 15$\,g sets a
lower limit near $\ux = 1.5$\,m/s, while droplet residence time sets an
upper limit near $\ux = 5.0$\,m/s, for an unaided \textbf{turndown of
$\approx 3.3{:}1$}.  The three-nozzle liquid feed extends liquid-side
turndown independently---each nozzle can be valved, so the minimum
stable sump head ($9$\,mm for three nozzles) still sits far below the
50--100\,mm operating head.  The documented TD cases (jet flood
25--29\,\%) fall comfortably inside the CIST envelope: at those rates
the CIST runs at the low end of its $\ux$ band with the swirl field
still above the $15$-g floor, whereas the vendor decks are by then near
their own weep/low-load margins.

\paragraph{Efficiency.}  On the O'Connell basis the curved-vane,
three-nozzle CIST reaches the 95\,\% cap in all twelve cases against
$\approx 77$--80\,\% for the valve decks.  For a fixed number of
theoretical stages this translates to roughly \textbf{15\,\% fewer
actual trays than UFM and 19\,\% fewer than SVG}.  Combined with the
24-in spacing retained from the vendor design, the shorter tray count
and the much smaller diameter give a substantially smaller and lighter
column for the identical separation.

\paragraph{Caveat.}  These CIST figures are model predictions from the
first-principles correlations of this disclosure, not measured data;
the vendor numbers are rated values from commercial software.  The
efficiency multiplier (1.32) and the pressure-drop coefficient
($K = 1.73$) are the two parameters most in need of pilot confirmation
(Section~\ref{sec:limitations}).  The diameter reduction, which follows
directly from the chamber-count geometry and is largely insensitive to
those two parameters, is the most robust of the four results.
...
\textbf{Claim 15.}
The tray of Claim~1 further comprising at least one of: (a) a
\emph{variable-pitch} helical guide vane affixed to the inner
chamber wall whose helix angle $\theta(z) = \theta_0 +
(\theta_1-\theta_0)z/H_c$ increases monotonically from
$\theta_0 \in [15°, 25°]$ at the chamber base to
$\theta_1 \in [60°, 80°]$ at the crown, so that the imposed
tangential velocity $u_\theta(z) = u_x\cot\theta(z)$ is highest
where fresh droplets form and decreases smoothly toward zero at
the crown exit, thereby eliminating the near-axis dead core and
generating an integrated $N_{g,\mathrm{eff}} \ge 40$\,g; (b) an
annular ring-orifice baffle of inner diameter $0.7$--$0.9\,D_c$
affixed at height $H_c/2$ to create a localised high-shear zone
and secondary centrifugal collection stage.
